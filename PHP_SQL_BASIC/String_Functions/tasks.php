...
//      37.  Дана строка '12345678'. Сделайте из нее строку '12 345 678'.
$string_task37 = '12345678';
var_dump(number_format($string_task37, 0, '', ' '));


//      38. Нарисуйте пирамиду, как показано на рисунке, только у вашей пирамиды должно быть 9 рядов, а не 5.
//       Решите задачу с помощью одного цикла и функции str_repeat.
$string_task38 = 'x';
for ($i = 0; $i < 9; $i++) {
    echo str_repeat($string_task38, $i), "\n";
}

//      39. Нарисуйте пирамиду, как показано на рисунке. Решите задачу с помощью одного цикла и функции str_repeat.
for ($i = 0; $i < 9; $i++) {
    echo str_repeat($i, $i), "\n";
}

//      40. Дана строка 'html, <b>php</b>, js'. Удалите теги из этой строки.
$string_task40 = 'html, <b>php</b>, js';
echo strip_tags($string_task40), "\n";

//      41. Дана строка $str. Удалите все теги из этой строки, кроме тегов <b> и <i>
$string_task41 = 'my <b>string</b> <em>another</em> <i>my</i> string';
echo strip_tags($string_task41, '<b><i>'), "\n";


//      42. Дана строка 'html, <b>php</b>, js'. Выведите ее на экран 'как есть':
//       то есть браузер не должен преобразовать <b> в жирный.
$string_task42 = 'html, <b>php</b>, js';
echo htmlspecialchars($string_task42), "\n";


//      43.Узнайте код символов 'a', 'b', 'c', пробела
echo ord('a'), ord('b'), ord('c'), ord(' '), "\n";


//      44. Выведите на экран символ с кодом 33
echo chr(33), "\n";

//      45. Запишите в переменную $str случайный символ - большую букву латинского алфавита.
$string_task45 = chr(mt_rand(65, 90));


//      46. Запишите в переменную $str случайную строку $len длиной, состоящую из маленьких букв латинского алфавита.
$len = rand(1, 10);
$string_task46 = '';
for ($i = 0; $i < $len; $i++) {
    $string_task46 .= chr(mt_rand(97, 122));
}
echo $string_task46, "\n";
...
$char_task47 = 'A';
if (ord($char_task47) >= 97 && ord($char_task47) <= 122) {
    echo "Small", "\n";
} else if (ord($char_task47) >= 65 && ord($char_task47) <= 90) {
    echo "Big", "\n";
} else echo "Not a letter", "\n";

//      48. Дана строка 'ab-cd-ef'. С помощью функции strchr выведите на экран строку '-cd-ef'
$string_task48 = 'ab-cd-ed';
echo strchr($string_task48, '-'), "\n";

//      49. Дана строка 'ab-cd-ef'. С помощью функции strrchr выведите на экран строку '-ef'.
$string_task49 = 'ab-cd-ef';
echo strrchr($string_task49, '-'), "\n";


//      50. Дана строка 'ab--cd--ef'. С помощью функции strstr выведите на экран строку '--cd--ef'
$string_task50 = 'ab--cd--ef';
echo strstr($string_task50, '--'), "\n";

//      51. Преобразуйте строку 'var_test_text' в 'varTestText'.
//       Скрипт, конечно же, должен работать с любыми аналогичными строками
$string_task51 = 'var_test_text';
echo lcfirst(str_replace(' ', '', ucwords(str_replace('_', ' ', $string_task51)))), "\n";


//      52. Дана строка следующего вида: 5 цифр, затем пробел, затем еще 5 цифр.
//      Например, дана такая строка '12345 67890'. Сделайте из нее строку '54321 09876'
$string_task52 = '12345 67890';
$arr_task52 = explode(' ',$string_task52);
foreach ($arr_task52 as $key=>$element){
    $arr_task52[$key] = strrev($element);
}
echo implode(' ',$arr_task52),"\n";
echo '</pre>';